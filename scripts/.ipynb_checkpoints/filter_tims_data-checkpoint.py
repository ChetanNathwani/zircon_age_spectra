...
tims_data = pd.read_csv('zircon_tims_comp_filtered.csv',encoding = "ISO-8859-1")
...
filtered = filtered.groupby('Unit').filter(lambda x: (np.max(x['age68'])-np.min(x['age68']))/(np.mean(x['2s_68'])/2) >= dt_sigma_cutoff) # Filter those with length < threshold
